		...
		Clause deletion can remove vital information from a proof.
    	    ...
    	    \begin{column}{.45\textwidth}
                \begin{tikzpicture}[
                  ...
                  {
                     c_1 & \textcolor{red}{c_2}	& c_3 \\
                     x   & \textcolor{red}{y}      	&     \\
                         & \square 			&     \\
                     };
                  \draw [dep] (m-1-1) -- (m-2-1) -- (m-3-2);
                  \draw [dep,dash pattern=on 3mm off 3mm,red] (m-1-2) -- (m-2-2) -- (m-3-2);
                  \draw [dep] (m-1-3) -- (m-3-2);
                  \path[-stealth]
...
pivot          = 1\\
        ...
        %     \begin{longtable}[]{ll}
        %         Formula & SICK Certificate\tabularnewline
        %         \texttt{p\ cnf\ 2\ 2} &
        ...
        %         Incorrect DRAT Proof & \texttt{{[}{[}witness{]}{]}}\tabularnewline
        %         \texttt{1\ 0} & \texttt{failing\_clause\ =\ {[}-2,\ -1,\ {]}}\tabularnewline
        %         \texttt{0} & \texttt{failing\_model\ \ =\ {[}2,\ {]}}\tabularnewline
